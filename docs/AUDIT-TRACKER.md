...
| N-69 | Perf | Med | Warm cell "Surprise me" still ~4-5 s after N-52: HouseholdRestrictionResolver ran one ILIKE seq-scan per criterion pattern (62 for this household) over the 236k-row catalog | Fixed (v0.3.47 — pg_trgm GIN index on Ingredient.Name AND the name/alias OR split into two probes: OR-ing the correlated alias EXISTS forced a 200k-row seq scan per pattern regardless of the index. Measured: name probe 12 ms indexed, alias probe 0.1 ms, 62 patterns ≈ sub-second total. The v0.3.43 combined-regex attempt (20 s) is reverted) |
