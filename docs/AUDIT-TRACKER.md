...
| N-58 | Artifact (UI) | Med | Empty-state icons rendered as clipped fragments on every page (wedge/triangle/bars/square): `.nom-dashboard__empty-icon` (4rem) lost the specificity tie to Material's later-injected `.mat-icon` host style (24px box + overflow:hidden) | Fixed (v0.3.40 — `mat-icon.`-qualified selector + line-height) |
...
| N-61 | Bug (misleading) | Med | Dishes list count is unfiltered ("16 recipes") while the group page filters by visibility ("No recipes visible") — every group can be a dead end for a normal user | Open (needs visible-count or hide-empty-groups on the API list) |
...
| N-67 | Search quality | Low | Ingredient search is substring-based: "oats" surfaces "goats milk" and "six throats" alongside Rolled Oats — rank prefix/word-boundary first; "six throats" is junk catalog data | Open |
...
| N-71 | UX | Low | Profile Activity Level dropdown is alphabetized (Extremely Active first, Sedentary mid-list) — order by intensity | Open |
| N-72 | Mobile layout | Med | "New Cookbook" dialog overflows the right screen edge at 412 px (fields clipped) | Open |
| N-73 | Aesthetics | Low | Dashboard "Today's Nutrition" stat tiles stack one-per-row on phones — 2x2 grid would halve the scroll | Open |
| N-74 | Minor | Low | Context panel "Shopping for 2 upcoming recipes" while 3 meals incl. snacks are planned (snacks ingredients ARE in the list) — count may exclude snacks | Open |
