...
| N-68 | Data quality | Med | Word-quantity catalog residue survives the N-46 digit filter ("a pint of black beans", "one quart of boiling milk", …). Checked on prod: every such row IS referenced by imported recipes, so a purge means re-linking those RecipeIngredients to canonical rows first — a curation job, not a delete. (They are already hidden from search only when digit-prefixed; word-prefixed ones still appear) | Open |
