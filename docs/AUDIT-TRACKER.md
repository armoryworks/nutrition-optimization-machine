...
| N-79 | Search quality | Low | More residue classes hidden from search: word-quantity prefixes incl. compounds ("one half cup of …") and HTML-entity fractions ("&frac14; cup …") | Fixed (v0.3.51/53). Parenthetical prep-notes ("(rinsed and drained)", "(see above)") and comma-joined multi-ingredient rows still appear — same import-residue family as N-68, needs data curation |
| N-80 | UI papercut | Low | Phone autocomplete panel hid the top-ranked option: DevTools-over-adb showed the CDK overlay at panelTop -51px — the 256px default panel overflows the keyboard-shrunk viewport top, and mat-autocomplete neither pushes nor flexes (not scroll, not z-index — v0.3.48-54 chased those) | Fixed (v0.3.55 — panel max-height 176px on phones via token + !important; verified on device: "Black Beans" top, active, fully visible) |
| N-76 | Ops/auth | Med | Every nom-api redeploy signed ALL users out. Two causes: (a) Identity bearer tokens are Data Protection payloads and the DP key ring lived in the container FS (OIDC certs were fine — mounted PFXs); (b) the SPA cleared its session on ANY validate/refresh error, including 502s during the deploy window | Fixed (v0.3.44 — DP key ring persisted to auth.DataProtectionKeys via PersistKeysToDbContext; UI clears the session only on definitive 401/403 (validate) / 400-401 (refresh)). **API deploy requires the CREATE TABLE first** |
...
| N-65 | Mobile layout | Low | Pantry list header/rows: "INGREDIENTQUANTITY" ran together at 412 px | Fixed (v0.3.40 gap + v0.3.48 shorter labels Item/Qty/Expires — both columns shrank below their uppercase-tracked text, so gap alone was not enough; verified on device) |
...
| N-68 | Data quality | Med | Import residue in the catalog ("1 can black beans", "a pint of …"): **143k of 236k rows (60%)**, created continuously by the nightly harvest — the scraper hands the raw line as the ingredient name and `FindOrCreateIngredientAsync` minted a row verbatim (the N-28 refinement fixed only the recipe FORM's matching, never the import path) | Fixed (v0.3.56 — deterministic `IngredientNameNormalizer` at the import boundary stops new residue; new admin tool **Admin → Food Catalog → Residue cleanup** previews and applies merge/rename with recipe-link re-pointing, RawLine folding, and a guard that skips rows referenced by pantry/restrictions/nutrition/etc.) |
| N-69 | Perf | Med | Warm cell "Surprise me" still ~4-5 s after N-52: HouseholdRestrictionResolver ran one ILIKE seq-scan per criterion pattern (62 for this household) over the 236k-row catalog | Fixed (v0.3.47 — pg_trgm GIN index on Ingredient.Name AND the name/alias OR split into two probes: OR-ing the correlated alias EXISTS forced a 200k-row seq scan per pattern regardless of the index. **Verified on prod device: 651 ms cold, 503 ms warm server-side** — was ~10 s originally and 20 s with the v0.3.43 regex attempt, now reverted) |
